...
var fs			= require('fs'),
	...
	//	TODO: This should be index.js so that you just require the directory.
	// apiClientFile = 'index.client.js',
	// apiServerFile = 'index.js',
...
module.exports = function(app, options) {
	...
	execExternalScript = function(script, callback){
		...
		exec(script, function (error, stdout, stderr) {
			if(stdout) {
				options.verbose && console.log(stdout);
			}
			if(stderr) {
				options.verbose && console.log(stderr);
			}
			if(error) {
				options.verbose && console.log("BUILD exec ERROR: " + error);
			}
			callback();
		});
	};
